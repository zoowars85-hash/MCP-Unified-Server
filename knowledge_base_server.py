...
# -*- coding: utf-8 -*-
"""
MCP RAG Engine v1.1 – семантический индекс книг и файлов
Исправлено: rag_stats теперь показывает реальное количество файлов,
добавлена rag_list_files, улучшены метаданные.
"""
import os
import json
import hashlib
...
def rag_index_folder(folder_path: str, collection_name: str = "default",
    ...
    conversation_memory.add(
        ...
        status="completed",
        dialog=dialog_id,
        context=f"Indexed {indexed_chunks} chunks from {len(indexed_files)} files"
    )
...
def rag_stats(collection_name: str = "default", max_chunks: int = 10000) -> Dict:
    """
    Статистика индекса.
    Параметр max_chunks: сколько чанков просканировать для подсчёта уникальных файлов.
    """
    client = _get_client()
    try:
        coll = client.get_collection(collection_name)
        count = coll.count()
        # Получаем достаточно метаданных, чтобы покрыть все возможные файлы
        # (если чанков больше max_chunks, статистика может быть неточной, но для сотен файлов хватит)
        limit = min(max_chunks, count)
        if limit == 0:
            return {"collection": collection_name, "chunks": 0, "unique_files": 0}
        
        sample = coll.get(limit=limit, include=["metadatas"])
        unique_sources = set()
        if sample["metadatas"]:
...
def rag_list_files(collection_name: str = "default", limit: int = 100) -> Dict:
    """
    Возвращает список всех проиндексированных файлов в коллекции.
    Для каждого файла – количество чанков.
    """
    client = _get_client()
    try:
        coll = client.get_collection(collection_name)
        total_chunks = coll.count()
        # Получаем все метаданные (осторожно, если коллекция очень большая)
        # Для 10k+ чанков лучше использовать пагинацию, но для обычного использования хватит.
        all_data = coll.get(limit=total_chunks, include=["metadatas"])
        file_stats = {}
        ...
        sorted_files = sorted([{"source": src, "chunks": cnt} for src, cnt in file_stats.items()],
...
def register_tools(server: BaseMCPServer):
    ...
    server.register_tool("rag_stats", {
        "description": "Статистика индекса (количество чанков, уникальных файлов, модель)",
        "inputSchema": {
            "type": "object",
            "properties": {
                "collection_name": {"type": "string", "default": "default"},
                "max_chunks": {"type": "integer", "default": 10000}
            }
        }
    }, lambda **kw: rag_stats(kw.get("collection_name", "default"), kw.get("max_chunks", 10000)))
    ...
    server.register_tool("rag_list_files", {
        "description": "Список всех файлов, проиндексированных в коллекции, с количеством чанков",
        "inputSchema": {
            "type": "object",
            "properties": {
                "collection_name": {"type": "string", "default": "default"},
...
__mcp_plugin__ = {
    "name": "rag-engine",
    "version": "1.1",
    "description": "Векторный поиск и RAG для книг и документов (исправлена статистика, добавлен список файлов)",
    "dependencies": ["sentence_transformers", "chromadb", "pypdf", "docx", "ebooklib", "bs4"],
    "on_load": lambda: _log("[RAG] Engine v1.1 loaded. Use rag_index_folder to index your books."),
    "on_unload": lambda: _log("[RAG] Engine unloaded.")
}

if __name__ == "__main__":
    server = BaseMCPServer("rag-engine", "1.1")
